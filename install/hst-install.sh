...
check_wget_curl(){
    ...
    if [ -e '/usr/bin/wget' ]; then
        wget -q https://raw.githubusercontent.com/hestiacp/hestiacp/test_docker_ubuntu20.04/install/hst-install-$type.sh -O hst-install-$type.sh
        if [ "$?" -eq '0' ]; then
    ...
    if [ -e '/usr/bin/curl' ]; then
        curl -s -O https://raw.githubusercontent.com/hestiacp/hestiacp/test_docker_ubuntu20.04/install/hst-install-$type.sh
        if [ "$?" -eq '0' ]; then
            bash hst-install-$type.sh $*
            exit
        else
